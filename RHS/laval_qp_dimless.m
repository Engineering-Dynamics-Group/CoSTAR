...
function f = laval_qp_dimless(t,z,param)
  ...
  % t2 = t(2,:);                % t2 is the second row of the time vector
    ...
    f(3,:) = - 2*Di*(1+delta).*z3 - d3*e^2.*(z3.^3) - z1 + 2*Di*eta.*z2 + (eta^2).*sin(eta.*t1);
    f(4,:) = - 2*Di*(1+delta).*z4 - d3*e^2.*(z4.^3) - z2 - 2*Di*eta.*z1 + (eta^2).*cos(eta.*t1) - Fg/e;
