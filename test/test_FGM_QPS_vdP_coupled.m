...
options.opt_sol  = costaropts('stability','on','cont','on','auto_freq',auto_freq,'sol_type','qps','approx_method','fgm','act_param',active_parameter,'display','error-control');
options.opt_cont = costaropts('step_control','angle','direction',1,'predictor','tangent','subspace','pseudo-arc','mu_limit',eps_limit,'step_width',0.05,'max_cont_step',1e4);
options.opt_approx_method = costaropts('n_FFT',2^6,'phase_condition','int_poincare','n_hh_max',50);
options.opt_init = costaropts('c0',zeros(4,1),'cmatrix',c_max,'smatrix',s_max,'hmatrix',K3);
options.opt_stability = costaropts();
