...
param = {epsilon,s,mu_limit(1,2)};              % Parameter vector, all constant parameters are set here, the bifurcation parameter gets its starting value
...
options.opt_sol  = costaropts('stability','on','cont','on','non_auto_freq',non_auto_freq,'auto_freq',auto_freq,'sol_type','quasiperiodic','approx_method','shooting','act_param',active_parameter);
options.opt_cont = costaropts('step_control','corrector_iterations','predictor','tangent','subspace','pseudo-arc','mu_limit',mu_limit);
...
options.opt_stability = costaropts('iterate_bfp','on','n_char_st',50,'n_map',1e4);
