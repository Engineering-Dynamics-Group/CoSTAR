...
param = {mu_start,alpha,beta};                  % Parameter vector, all constant parameters are set here, the bifurcation parameter gets its starting value
...
options.opt_sol  = costaropts('stability','on','cont','on','auto_freq',auto_freq,'sol_type','quasiperiodic','approx_method','shooting','act_param',active_parameter,'display','full');
options.opt_cont = costaropts('step_control','off','predictor','parable','subspace','pseudo-arc','mu_limit',mu_limit);
options.opt_approx_method = costaropts('solver','ode45','n_char',75);
