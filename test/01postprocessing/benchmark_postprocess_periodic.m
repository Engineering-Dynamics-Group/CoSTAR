...
solget_options = costaropts('eval',@(z) z(1:100,1),'space','hypertime','index',index);               % Get half of the solution of z_1
solget_output  = S.solget(DYN,solget_options);

solget_options = costaropts('eval',@(z) max(vecnorm(z,2,2)),'space','hypertime');                   % This is the evaluation for contplot 'max2'
solget_output  = S.solget(DYN,solget_options);
...
contplot_options = costaropts('zaxis',@(z) max(z(:,1)),'Color',[0,0,0]);           % Plot bifurcation diagram with user-set function
contplot_output  = S.contplot(DYN,contplot_options);
...
solplot_options = costaropts('zaxis',@(z) z(:,1),'space','hypertime','resolution',200,'index',index(2:3),'Color','r');  % Plot solution z_1 in hypertime
solplot_output  = S.solplot(DYN,solplot_options);

solplot_options = costaropts('zaxis',@(z) z(:,1:2),'space','hypertime','resolution',500,'index',index(2));              % Plot solution z_1 and z_2 in hypertime
solplot_output  = S.solplot(DYN,solplot_options);

solplot_options = costaropts('zaxis','all','space','hypertime','resolution',200,'index',index(2:3));                    % Plot all solutions in hypertime
solplot_output  = S.solplot(DYN,solplot_options);
...
solplot_options = costaropts('zaxis',@(z) z(:,1:2),'space','time','index',index(2),'interval',[0,10],'linestyle','--','figure',gcf);    % Plot interval of time solution z_1 of specific index
solplot_output  = S.solplot(DYN,solplot_options);
...
solplot_options = costaropts('zaxis',@(z) z(:,1:2),'space','frequency','index',index(3),'resolution',2^10,'interval',[0,10*2*pi/S.freq(index(3))]);     % Plot frequency spectrum of z_1 of specific index
solplot_output  = S.solplot(DYN,solplot_options);
