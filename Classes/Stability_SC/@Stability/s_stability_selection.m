...
%         end
        ...
        %%%%%% Multiple Shooting %%%%%%%%
        ST = ST_PS_MSHM(DYN);                    % Stability computation of periodic solutions via SHM (possible for FDM and FGM as well!)

        if(DYN.n_auto==0)
            ST.calc_stability = @(y,J) ST.PS_MSHM_calc_stability_non_auto(y,J,DYN,AM);   % Stability function for non-autonomous single shooting
        elseif(DYN.n_auto==1)
            ST.calc_stability = @(y,J) ST.PS_MSHM_calc_stability_auto(y,J,DYN,AM);       % Stability function for autonomous single shooting
        end
