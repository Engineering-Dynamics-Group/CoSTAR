...
function help_struct = s_help_opt_sol()
    ...
    help_struct.optional.approx_method.text   = ['Defines the approximation method used for the computation of periodic or quasi-periodic solutions. You can either use the whole word (e.g. ''fourier-galerkin'') or its abbreviation (e.g. ''fgm'').\n' ...
                                                 '-> NOTE: The field is not allowed for equilibrium solutions.'];

    help_struct.optional.act_param.value  = 'positive integer (scalar) \n <= length(system.param) \n e.g.: 3';
    ...
    help_struct.optional.non_auto_freq.value  = '[1x1] or [1x2] double or \n function handle @(mu) array \n e.g.: @(mu) [mu, pi*mu]';
    help_struct.optional.non_auto_freq.text   = ['Defines the non-autonomous frequency(s) (naf) of a (quasi-)periodic solution. \n' ...
                                                 '-> Must be function handle if the naf depend(s) on the continuation parameter mu.\n' ...
                                                 '-> Can be function handle or double if the naf are constant thoughout the continuation.\n' ...
    ...
    help_struct.optional.auto_freq.text   = ['Defines an initial guess for the autonomous frequency(s) (af) of a (quasi-)periodic solution. \n' ...
                                             '-> The sum of non-autonomous and autonomous frequencies (naf + af) must correspond to the solution type,' ...
                                             '    i.e. naf + af = 1 for periodic and naf + af = 2 for quasi-periodic solutions.\n' ...
                                             '-> NOTE: The field is not allowed for equilibrium solutions.'];
        
    help_struct.optional.freq_limit.value = 'positive double (scalar) \n e.g.: 1e-3 \n Default: 1e-4';
    help_struct.optional.freq_limit.text  = ['Defines the bottom limit of allowed base frequency values (frequency limit) of a solution. CoSTAR stops if any base frequency falls below the limit.\n' ...
                                             '-> NOTE: The field is not allowed for equilibrium solutions.'];

    help_struct.optional.display.value  = '''off'', ''final'', \n ''iter'', ''iter-detailed'', \n ''step-control'',\n ''error-control'' or ''full'' \n Default: ''iter''';
    help_struct.optional.display.text   = ['Controls the command window output.\n ' ...
                                           '--> ''off'': Only warnings and errors are displayed.\n ' ...
                                           '--> ''final'': The latest solution and the termination message are displayed.\n' ...
                                           '--> ''iter'': Displays the iteration process of the initial solution, the latest five solutions during continuation and the termination message.\n' ...
                                           '--> ''iter-detailed'': Similar to ''iter'', but all computed solutions are displayed.\n' ...
                                           '--> ''step-control'': Similar to ''iter-detailed'', but with additional step control information (if enabled).\n' ...
