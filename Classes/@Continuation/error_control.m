...
function obj = error_control(obj,S,AM,DYN)
    ...
    if obj.p_error_flag == 0    % In case of a critical error:
        obj.p_error = NaN;      % Set error to NaN ...
        return                  % and skip the error control (return to m_continuation)
    end
    ...
    while iterate
        ...
        if iterate == 1
            ...
            [y1_ec,~,newton_flag_ec,output_ec,J1_ec] = fsolve(Fcn,obj.yp,obj.fsolve_opts);  % Solve corrector-function
            ...
            if any(newton_flag_ec == [1,3,4])                               % Accept the result from fsolve if exitflag is 1, 3 or 4
                ...
                if decrease
                    err = AM.IF_estimate_error(y1_ec,DYN);
                ...
                obj.p_y1 = y1_ec;
                obj.p_newton_flag = newton_flag_ec;
                obj.p_output = output_ec;
                obj.p_J1 = J1_ec;
            ...
            % Exit flag handling Ib: If exitflag from fsolve is <= 0 or = 2: Issue warning, reduce step width (if possible - done below) and terminate error control
            else
                warn_text = append('Error control failed during iteration ',num2str(counter),' for solution Iter = ',num2str(obj.p_local_cont_counter+1),', because fsolve returned exit_flag = ',num2str(newton_flag_ec),'!');
        ...
        elseif iterate == -1
            obj.IF_ec_save_reset_props(AM,'reset');                         % Reset the modified properties -> needed to continue since there is no new solution with updated discretization
            warn_text = append('Increase of discretization failed during error control iteration ',num2str(counter),' for solution Iter = ',num2str(obj.p_local_cont_counter+1),'!');
        ...
        elseif iterate == 0
            ...
            break               % We can return to m_continuation now and can skip Exit flag handling II
        ...
        %%%%%%%%%% Exit flag handling II: Reduce step width or terminate %%%%%%%%%%
        if iterate == -1 || ~any(newton_flag_ec == [1,3,4])

            % Reduce step width if possible
            if obj.step_width > obj.step_width_limit(1,1)
                ...
                info_text = append('Computing a new solution with reduced step width. Step width adapted to stepwidth = ',num2str(obj.step_width),'.');
                write_log(DYN,info_text)                                                                    % Write info text in log file
                if strcmpi(DYN.display,'step-control') || strcmpi(DYN.display,'full')
                    disp(info_text)                                                                         % Display info text
                end
                obj.p_error_flag = 2;                                                                       % Set error flag to continue with the next continuation loop
            
            % Step width has reached minimal value: stop continuation
            else
                obj.p_error_msg = append('ERROR: ',warn_text);                                              % Set error message based on the warning text defined above
                obj.p_stopping_flag = 'CoSTAR stopped because error control failed and step width has reached minimal value.';   % Set stopping message
                obj.p_error_flag = 0;                                                                       % Set error flag to stop the continuation
