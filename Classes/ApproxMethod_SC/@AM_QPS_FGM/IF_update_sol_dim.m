%This method updates the dimension of the solution data of past curve points, if the solution space dimension has been changed due to the error_control method
% or within the iteration process for the bifurcation point (Stability class )
%
...
function varargout = IF_update_sol_dim(obj,DYN,new_dim,varargin) 
    ...
    elseif new_dim<numel(y0)
        ...
        tmp_hh = setdiff(hhmold.',obj.hmatrix.','rows').';
        if ~isempty(tmp_hh); idx = find(prod(hhmold == tmp_hh,1)); idx = idx-1;  else;  idx = [];  end
        if isempty(idx); idx = size(cmatrix,2)-1; obj.hmatrix = hhmold(:,1:end-1); end %If something goes wrong - simply delete the last harmonic
        idx2= [1:(idx-1),(idx+1):size(hhmold,2)-1];
