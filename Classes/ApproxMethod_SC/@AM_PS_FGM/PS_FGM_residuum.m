...
function res = PS_FGM_residuum(obj,y,DYN)                            
    ...
    if isfield(DYN.system,'first_integral')
        ...
        I_Z = I(FS,param);                                          % Evalute the first integral for all evaluation points
        IC = 1/n_fft*sum(I_Z) - param{end};                         % First Integral Constraint: I(s) = param{end} | Take the average of I_Z to get a single value for all evaluation points
