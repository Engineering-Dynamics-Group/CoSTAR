...
%
%@obj: Approximation Method AM_PS_FGM object
...
function [yp,flag] = IF_decrease_discretization(obj,y1,DYN)
    ...
    if numel(obj.hmatrix)>2
        ...
        for k =2:(numel(hhm0)-1)    %Delete iteratively one frequency to find the frequency with the lowest impact on the error, which is then deleted.
            ...
            err(1,k-1) = obj.IF_estimate_error(ytmp,DYN);
        end
        ...
        if isempty(idx_min)     %Check if an idx was found, there are any inf's or nan's and if the new matrix is unique
            ...
            if (max(obj.hmatrix) <= (obj.n_fft/4-2)) && (obj.n_fft > n_fft_min)     % Halve n_fft in accordance with Nyquist-Shannon and if current n_fft is larger than minimum n_fft
