...
function help_struct = s_help_opt_init_QPS_SHM()
    ...
    help_struct.info = ['opt_init --- quasiperiodic solution --- shooting method \n ' ...
                        'There are two possibilities to set the initial value:\n Either iv OR c0, c1_matrix and/or s1_matrix can be used. \n\n' ...
    ...
    help_struct.optional.iv.text             = ['Possibility 1: Defines the initial value for the nonlinear system solver fsolve, without autonomous frequency(s) and continuation parameter (i.e. a method solution vector s can be used for iv).\n' ...
                                                'If numel(iv)/dim does not match the value of the opt_approx_method field ''n_char'' (or its default value), iv is interpolated.\n' ...
                                                'The fields c0, c1_matrix and s1_matrix are not allowed when providing iv.'];

    help_struct.optional.c0.value   = '[dim x 1] array \n e.g.: [1;0] \n Default: zeros(dim,1)';
    help_struct.optional.c0.text    = 'Possibility 2: The initial values are calculated using a first order multi- dimensional Fourier series. c0 defines the constant Fourier series coefficient.\nThe field iv is not allowed when providing c0.';

    help_struct.optional.c1_matrix.value   = '[dim x 1], [dim x 2] or \n [dim x 3] array \n e.g.: [1;0], [1,1;0,0] or \n [1,1,1;0,0,0] \n Default: zeros(dim,3)';
    help_struct.optional.c1_matrix.text    = ['Possibility 2: The initial values are calculated using a first order multi- dimensional Fourier series. \n'...
                                              ...
                                              'Non-supplied columns are treated as zero vectors.\nThe field iv is not allowed when providing c1_matrix.'];

    help_struct.optional.s1_matrix.value   = '[dim x 1], [dim x 2] or \n [dim x 3] array \n e.g.: [0;-1], [0,0;-1,-1] or \n [0,0,0;-1,-1,-1] \n Default: zeros(dim,3)';
    help_struct.optional.s1_matrix.text    = ['Possibility 2: The initial values are calculated using a first order multi- dimensional Fourier series. \n'...
                                              ...
                                              'Non-supplied columns are treated as zero vectors.\nThe field iv is not allowed when providing s1_matrix.'];
