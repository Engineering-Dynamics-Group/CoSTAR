...
function [res,J_res] = PS_SHM_residuum(obj,y,DYN)
    ...
    Z_p = obj.Z0{1};                                    % Get the trajectory of the preceding solution for evaluating the integral phase condition
    % Z_p_mu_plus = obj.Z0{2};                            % Get the trajectory at the predictor point for "+ delta" perturbed mu
    % Z_p_mu_minus = obj.Z0{3};                           % Get the trajectory at the predictor point for "- delta" perturbed mu
    ...
    if (n_auto == 1)  && ~strcmpi(obj.phase_condition,'off')                                            % Compute the phase condition if system is autonomous
        switch obj.phase_condition                                                                      % The phase condition is always present for dissipative systems and can be present for conservative systems
            case 'poincare'
            ...
            case 'integral'
                ...
                pc = f_p.' * (reshape(Z_traj,dim*n_shoot*n_time,1) - reshape(Z_p,dim*n_shoot*n_time,1));% * T / (n_time*n_shoot); % Integral phase condition
        end
   ...
   res = [s_end - s_perm;  pc;  IC];                    % Assemble the residuum: Residuum of the shooting; phase condition; first integral constraint
    ...
    % Autonomous system: Calculate the derivative of g with respect to omega
    if n_auto == 1
    ...
    % Non-Autonomous system: Set dg/domega as empty in order to add "nothing" to the Jacobian matrix below
    elseif n_auto == 0
        dg_domega = [];  
    end

    % If phase condition is enabled (always the case for autonomous dissipative systems, can be the case for conservative systems): Calculate dpc/dy
    if (n_auto == 1) && ~strcmpi(obj.phase_condition,'off')
        switch obj.phase_condition 
            ...
            case 'integral'                     % Integral phase condition
                ...
                %     f_p_k = f_p((k-1)*dim*n_time+1:k*dim*n_time);                   % Part of f (Fcn evaluated for all ~100 evaluation points) that belongs to k-th interval -> size: [dim*n_time x 1]
                ...
                %     dpc_ds(1,(k-1)*dim+1:k*dim) = f_p_k.' * dz_j_dz_k;                                              % dpc/dz_k
                %     dpc_domega = dpc_domega - 2*pi/(n_time*n_shoot*omega^2) * f_p_k.'*(j_vec.*f_k);                 % dpc/domega
                % end
                ...
                dz_j_dz_k = reshape( permute( reshape(dz_j_dz_k,dim,dim,n_time,n_shoot), [1 3 2 4] ), dim*n_time, dim*n_shoot );    % We need this particular structure for the multiplikation with f_p
                % dz_j_dz_k stores the n_time*n_shoot [dim x dim] matrices dz_j/dz_k in the k-th "column" and the j-th "row". The reshape(f_p,...).' below stores in the k-th row the f(z_p_{k,j}) for all j 
                f_p_dz_j_dz_k = reshape(f_p,dim*n_time,n_shoot).' * dz_j_dz_k;      % This product evaluates all f(z_p_{k,j}).' * dz_j/dz_k (for all combinations of the two k). However, we only need products of matching k
                ...
                dpc_ds = reshape(f_p_dz_j_dz_k(eval_mat),1,dim*n_shoot);            % dpc/ds | The reshape is needed because a row vector is returned for n_shoot = 1, while a column vector is returned for > 1
                ...
                dpc_domega = - 2*pi/(n_time*n_shoot*omega^2) * f_p.'*(J_vec.*f);    % dpc_domega
                % dpc_dmu:
                if isfield(DYN.system,'first_integral') && (DYN.act_param == numel(param))  % If the value of the first integral is the continuation parameter
                    dpc_dmu = 0;                                                    % dpc/dmu is zero since pc is not explicitly dependent on I = mu
                else
                    if calc_stability
                        df_p_dmu = (Fcn(0,reshape(Z_p,dim,n_time*n_shoot),param_mu_plus) - Fcn(0,reshape(Z_p,dim,n_time*n_shoot),param_mu_minus)) ./ (2*delta_mu);
                        dz_j_dmu = (reshape(Z_traj_mu_plus,dim,n_time*n_shoot) - reshape(Z_traj_mu_minus,dim,n_time*n_shoot)) ./ (2*delta_mu);
                    else
                        df_p_dmu = (Fcn(0,reshape(Z_p,dim,n_time*n_shoot),param_mu_plus) - Fcn(0,reshape(Z_p,dim,n_time*n_shoot),param)) ./ delta_mu;
                        dz_j_dmu = (reshape(Z_traj_mu_plus,dim,n_time*n_shoot) - reshape(Z_traj,dim,n_time*n_shoot)) ./ delta_mu;
                    end
                    dpc_dmu = reshape(df_p_dmu,dim*n_time*n_shoot,1).' * reshape(Z_traj - Z_p,dim*n_time*n_shoot,1) + f_p.' * reshape(dz_j_dmu,dim*n_time*n_shoot,1);
                end
                % OLD INTEGRAL PHASE CONDITION (USES f INSTEAD OF f_p)
                ...
                %     j_vec = reshape(repmat(0:n_time-1,dim,1),dim*n_time,1);         % Create a vector containing dim times 0, dim times 1, ..., dim times (n_time-1)
                %     dpc_domega = dpc_domega - 2*pi/(n_time*n_shoot*omega^2) * ((df_dz_j_mat*f_k).' * (j_vec.*(Z_traj_k - Z_p_k)) + f_k.'*(j_vec.*f_k));  
                % end
                % dpc_dmu:
                % if isfield(DYN.system,'first_integral') && (DYN.act_param == numel(param))  % If the value of the first integral is the continuation parameter
                %     dpc_dmu = 0;                                                    % dpc/dmu is zero since pc is not explicitly dependent on I = mu
    ...
    % Phase condition is missing: Set dpc/dy as empty in order to add "nothing" to the Jacobian matrix below
    else
