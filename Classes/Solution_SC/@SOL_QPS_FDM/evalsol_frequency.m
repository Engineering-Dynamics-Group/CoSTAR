...
% It (re-)calculates a 1-dimensional FFT of the time solution. The resolution of the time analysis is options.resolution. 
% The FFT returns the absolute value of the complex fourier transform and applies a rectangular leackage window.
...
function [s,a,mu,omega] = evalsol_frequency(obj,DYN,options)
    ...
    [s_time,mu,time] = obj.evalsol_time(DYN,options);                   % Get the time dependent solutions
                                                                        % In the future: Do a 2D-FFT of the hypertime manifolds?
