% Method of SOL_QPS_SHM: This method (re-)calculates a FFT of the periodic orbit data of a shooting solution
...
function [s,mu,f] = evalsol_frequency(obj,DYN,options)
    ...
    [s_time,mu,time] = obj.evalsol_time(DYN,options);                       % Get the time solution
    
    for k = 1:numel(mu)
        [f_tmp,s_tmp]   =  costarFFT(time(:,1,k),s_time(:,:,k));            % Compute the Fourier-Transform for all mu
        f(:,1,k) = 2.*pi.*f_tmp.';                                          % Multiply by 2pi to get angular frequency
        s(:,:,k) = s_tmp;
    end
