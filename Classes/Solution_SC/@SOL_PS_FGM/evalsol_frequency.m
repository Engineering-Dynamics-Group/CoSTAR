...
function [P,a,mu,f] = evalsol_frequency(obj,DYN,options)
    ...
    % Check the resolution: We want to return the data at least for omega = [0, 1, 2, ... , max(hmatrix)+1].*freq
    max_h_max = max(cellfun(@max,obj.hmatrix(1,index)));    % Get the highest harmonic of all solutions to be evaluated
    if res < 2*(max_h_max+2)        % We need at least max_h_max+2 (+2 for 0 and max(hmatrix)+1) data points to return, hence the resulution must be at least 2*(max_h_max+2)
        warning(append('A problem occured during the evaluation of your FGM solution(s) in frequency space. ', ...
                       ...
                       'The resolution was therefore increased to the minimum required resolution of 2*(max(h_max)+2) = ', num2str(2*(max_h_max+2)),', ', ...
                       'where h_max is a vector of the maximum harmonics of all solutions to be evaluated.'));
        res = 2*(max_h_max+2);      % Overwrite the resolution, because a smaller value for res does not make sense and the code will probably not work
    end
    ...
    for k = index
        ...
        alpha = atan2(imag(FC),real(FC));           % Phase angles = atan2(-S/C) = atan2(-(-imag(S))/real(S))           
