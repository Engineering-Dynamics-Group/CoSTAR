...
% the solution. We will learn how to set up CoSTAR and how to correctly define all the required settings on the basis of three ...
% different examples:
%
...
% to (2). The time t as well as the state variable vector z are always treated seperately and therefore not included in param. ...
...
options.opt_sol = costaropts('sol_type','quasiperiodic','approx_method','fourier-galerkin','cont','on','stability','off', ...
...
%                                       calculation is not available yet when using the FGM, so we have to set it to 'off'.
...
%                  
% So far we have definied all "options" structures which CoSTAR always needs. Since we want to calculate a quasi-periodic solution ...
...
% type "costarhelp.<name_of_options_structure>('QPS','FGM')" ("QPS": Quasi-Periodic Solution).
...
% essentially a projection of the curve in R^(n+1)-dimensional space into 2D space.
...
opt_solplot = costaropts('zaxis',@(z) z(:,:,1),'space','hypertime','resolution',30,'index',[25,200]);
...
% Here, solplot returns the desired plot as well as the corresponding hyper-time coordinates theta_Duff, the Z_1-values Z_1_Duff ...
% and the corresponding mu-values mu_Duff. However, the output of solplot depends on the options, so it may change in other cases.
[theta_Duff,Z_1_Duff,mu_Duff] = S.solplot(DYN,opt_solplot);
%
...
options_kappa.opt_sol = costaropts('sol_type','quasiperiodic','approx_method','fourier-galerkin','cont','on','stability','off', ...
...
% calculating quasi-periodic solutions. param is a cell array storing all parameters, apart from t and z, that need to be passed ...
% to (5). The time t as well as the state variable vector z are always treated seperately and therefore not included in param. ...
...
%                                    Step control is detailly explained in a separate tutorial.
...
% of (64*64) torus function points is calculated. These points are equidistant distributed on the hyper-time domain. After the ...
% Euclidean norm of all torus function points were computed, the maximum thereof is taken and plotted against mu. This is ...
% essentially a projection of the curve in R^(n+2)-dimensional space into 2D space.
...
% In order to plot the hyper-time surfaces Z_i(theta_1,theta_2,mu) (torus function components), we need to call the "solplot" ...
% function. Similar to the "costar" function, solplot expects a structure that defines all required options for the plot.
opt_solplot = costaropts('zaxis',@(z) z(:,:,1),'space','hypertime','resolution',30,'index',[5,50]);
% Mandatory fields: - 'zaxis':       Defines what is plotted on the vertical axis. "@(z) z(:,:,1)" plots the first component (i = 1) ...
%                                    of the torus function against the hyper-time domain [0, 2*pi]^2.
%                   - 'space':       Specifies the "domain" of the plot. 'hypertime' plots the hyper-time surfaces of the solution.
% Optional fields:  - 'resolution':  Sets the number of points for the plot in each theta-direction.
...
% Now we can call the "solplot" function. It is a function of the solution class object S, which is why we have to call it by ...
% "S.solplot". Apart from the "opt_solplot" structure, solplot also requires the DynamicalSystem class object DYN. 
% Here, solplot returns the desired plot as well as the corresponding hyper-time coordinates theta_JLr, the Z_1-values Z_1_JLr ...
% and the corresponding mu-values mu_JLr. However, the output of solplot depends on the options, so it may change in other cases.
[theta_JLr,Z_1_JLr,mu_JLr] = S.solplot(DYN,opt_solplot);
%
% Further postprocessing options and functions can be found via the costarhelp function (type "costarhelp.costar" in the command ...
% window) since a complete explanation of these would exceed the scope of this tutorial.
...
% epsilon is the coefficient of the nonlinearity, alpha the coupling constant, beta a constant of the linear stiffness of x2, ...
...
% calculating quasi-periodic solutions. param is a cell array storing all parameters, apart from t and z, that need to be passed ...
% to (8). The time t as well as the state variable vector z are always treated seperately and therefore not included in param. ...
...
% Apart from the information in the command window, CoSTAR plots the maximum of the Euclidean norm of the torus function against ...
% the continuation parameter mu. This means that for every computed approximate solution Z(theta_1,theta_2,mu), the Euclidean norm ...
% of (200*200) torus function points is calculated. These points are equidistant distributed on the hyper-time domain. After the ...
% Euclidean norm of all torus function points were computed, the maximum thereof is taken and plotted against mu. This is ...
% essentially a projection of the curve in R^(n+3)-dimensional space into 2D space.
...
% In order to plot the hyper-time surfaces Z_i(theta_1,theta_2,mu) (torus function components), we need to call the "solplot" ...
% function. Similar to the "costar" function, solplot expects a structure that defines all required options for the plot.
opt_solplot = costaropts('zaxis',@(z) z(:,:,1),'space','hypertime','resolution',40,'index',[5,55]);
% Mandatory fields: - 'zaxis':       Defines what is plotted on the vertical axis. "@(z) z(:,:,1)" plots the first component (i = 1) ...
%                                    of the torus function against the hyper-time domain [0, 2*pi]^2.
%                   - 'space':       Specifies the "domain" of the plot. 'hypertime' plots the hyper-time surfaces of the solution.
% Optional fields:  - 'resolution':  Sets the number of points for the plot in each theta-direction.
...
% Now we can call the "solplot" function. It is a function of the solution class object S, which is why we have to call it by ...
% "S.solplot". Apart from the "opt_solplot" structure, solplot also requires the DynamicalSystem class object DYN. 
% Here, solplot returns the desired plot as well as the corresponding hyper-time coordinates theta_cvdP, the Z_1-values Z_1_cvdP ...
% and the corresponding mu-values mu_cvdP. However, the output of solplot depends on the options, so it may change in other cases.
[theta_cvdP,Z_1_cvdP,mu_cvdP] = S.solplot(DYN,opt_solplot);
%
% Further postprocessing options and functions can be found via the costarhelp function (type "costarhelp.costar" in the command ...
% window) since a complete explanation of these would exceed the scope of this tutorial.
