...
%
% In this tutorial, we will look at how to compute quasi-periodic solutions using the Fourier-Galerkin Method (FGM) to approximate ...           
% the solution (associated example: Example_QPS_FGM). We will learn how to set up CoSTAR and how to correctly define all the required ...
...
%                         - Usage of the "options.opt_approx_method"-fields 'n_FFT', 'error_control', 'error_limit', 'ec_iter_max' ...
%                           and 'n_hh_max' (in sections 1.2 and 1.4)
%
...
%
% Our aim is to approximate the quasi-periodic solution of equation (2) z(tau) for eta in the range of [0.8, 2] using the FGM. ...
...
% approximate discrete points on this curve by executing a path continuation. As the frequency eta is varied, the ...
...
%
Fcn =  @(t,z,param) duffing_qp(t,z,param);      % Fcn is a function handle containing the right-hand side (RHS) of equation (2). 
...
%
% TASK: Open the function "duffing_qp" (click the right mouse button right on it and choose 'Open "duffing_qp"' or go into ...
...
% correspond to the definitions in the "duffing_qp" function! For example, kappa is defined by kappa = param{2} in the ...
% "duffing_qp" function. Therefore, kappa has to be the SECOND variable in the "param" array. For the variable eta, we use eta0 ...
...
% Z0 = Z(theta_1,theta_2,mu0). Using the FGM, the initial value is created via a multi-dimensional Fourier series:
% Z_IV (theta_1,theta_2) = sum_(k=0)^(n_hh-1) ( C_k*cos(h_1_k*theta_1+h_2_k*theta_2) + S_k*sin(h_1_k*theta_1+h_2_k*theta_2) ).
% The h_i_k (i = 1,2) are taken from Hmatrix = [h_1_0, h_1_1, ... , h_1_(n_hh-1);
%                                               h_2_0, h_2_1, ... , h_2_(n_hh-1)]
% which stores the harmonics to be used. Hmatrix must at least contain [0,1,0; 0,0,1]. Therefore, Z_IV is at least ...
% Z_IV(theta_1,theta_2) = C_0 + C_1*cos(theta_1) + S_1*sin(theta_1) + C_2*cos(theta_2) + S_2*sin(theta_2). ...
...
% Smatrix and C0 must match the dimension of the system. The number of columns of Cmatrix and Smatrix must be at least two and may ...
...
% are not enough higher harmonics), it may happen that fsolve does not converge at all! In general, it can be quite challenging to ...
% find suitable values for the parameters of the initial value. For our Duffing example, we use:
Hmatrix = [0, 1, 0, 3, 0;       % NOTE: This Hmatrix will only lead to a good approximation if the error control is used ...
           0, 0, 1, 0, 3];      %       (we will use it).
C0 = [0; 0];
Cmatrix = [0.1,  0.1;           % The coefficients C_3 and C_4 corresponding to Hmatrix(:,4:5) will be guessed            
           0.08, 0.08];
Smatrix = [ 0.1,   0.1;         % The coefficients S_3 and S_4 corresponding to Hmatrix(:,4:5) will be guessed
           -0.08, -0.08];
...
% field ('auto_freq') is needed when the system exhibits autonomous frequency(s). Apart from that, there are four optional ...
% fields, but we use their default values and therefore do not set them.
options.opt_sol = costaropts('sol_type','quasiperiodic','approx_method','fourier-galerkin','cont','on','stability','on', ...
...
% type as well as the chosen approximation method. For quasi-periodic solutions using the FGM, there are two possibilities to ...
% define the initial value: Either the fields 'c0', 'cmatrix', 'smatrix' and 'hmatrix' are specified (four mandatory fields) OR the ...
% fields 'fc0' and 'hmatrix' are set (two mandatory fields). There are no optional fields.
...
%                                 the first approximate solution Z0(theta_1,theta2,mu0). Must at least contain the values ...
%                                 [0,1,0; 0,0,1]. The ordering of the harmonics must correspond to the ordering of 'cmatrix' and ...
%                                 'smatrix' or 'fc0'. We already defined Hmatrix above.
%                                 -> Allowed values: [1 x n_hh] integer array with n_hh = number of harmonics
%                   - 'c0':       Sets the Fourier series coefficient of the constant term used to create an initial value for fsolve ...
%                                 to find the first approximate solution Z0(theta_1,theta2,mu0). We already defined C0 above.
...
%                                 fsolve to find the first approximate solution Z0(theta_1,theta2,mu0). The k-th column of 'cmatrix' ...
%                                 corresponds to the (k+1)-th column of 'hmatrix' (the ordering must match the ordering in 'hmatrix'). ...
%                                 If the number of columns is less than (n_hh-1), the missing coefficients are guessed based on an ...
%                                 exponential decay of the norm of the coefficients. We already defined Cmatrix above.
%                                 -> Allowed values: [dim x n] (double) array with 2 <= n <= (n_hh-1) = size(hmatrix,2)-1
%                   - 'smatrix':  Sets the Fourier series coefficients of the sine terms used to create an initial value for ...
%                                 fsolve to find the first approximate solution Z0(theta_1,theta2,mu0). The k-th column of 'smatrix' ...
%                                 corresponds to the (k+1)-th column of 'hmatrix' (the ordering must match the ordering in 'hmatrix'). ...
%                                 If the number of columns is less than (n_hh-1), the missing coefficients are guessed based on an ...
...
%                                 approximate solution Z0(theta_1,theta2,mu0). The ordering in 'fc0' must correspond to the ordering ...
%                                 of the harmonics specified in 'hmatrix'. If the number of coefficients placed in 'fc0' is less than ...
...
%                                 -> Allowed values: [dim*(2*n+1) x 1] (double) array with 2 <= n <= (n_hh-1) = size(hmatrix,2)-1
% NOTE: For usage of the field 'fc0', please see section 1.4. 
%                  
...
% quasi-periodic solutions using the FGM, there are no mandatory fields and six optional field.
options.opt_approx_method = costaropts('n_FFT',2^5,'error_control','on','error_limit',[1e-4,1e-2],'ec_iter_max',10,'n_hh_max',50);
% Optional fields: - 'n_FFT':          Sets the number of evaluation points along each theta-direction for the Fast Fourier ...
%                                      Transformation. For ideal performance of the FFT, only "power of two"-values should be used. ...
%                                      A high value leads to higher accuracy but also to higher numerical effort.
...
%                                      -> Allowed values: Positive integer
%                                      -> Default value:  Inf
%                  - 'phase_condition': Defines the type of phase condition used for calculating solutions of systems exhibiting one ...
%                                       or two autonomous frequencies. 'int_poincare' phase condition is recommended. For two ...
%                                       autonomous frequencies, only "int_poincare" is possible. 'phase_condition' is not allowed when ...
%                                       dealing with full non-autonomous systems.
%                                       -> Allowed values: 'poincare', 'int_poincare'
%                                       -> Default value:  'int_poincare'
% NOTE: It is not necessary to set 'error_control', 'ec_iter_max' and 'phase_condition' since the default values are used. However, it ...
...
%
% Additionally, we set one field of the "options.opt_stability" structure. In general, this is not necessary when computing the ...
% stability of solutions as this options structure only contains optional fields. However, we set the field 'n_char_st' here so that ...
% the stability computation is more reliable for this example.
options.opt_stability = costaropts('n_char_st',25);
% Optional fields: - 'n_char_st':  Defines the number of characteristics of the quasi-periodic shooting method used to obtain the ...
%                                  mapping matrices. More characteristics improve the accuracy of the computed Ljapunov exponents. ...
%                                  Use a high number for complex manifolds. Please keep in mind that stability computation is ...
%                                  adressed in a separate tutorial in detail.
%                                  -> Allowed values: Positive integer
%                                  -> Default value:  100
%
...
% and "options.opt_stability", type "costarhelp.<name_of_options_structure>('QPS','FGM')" ("QPS": Quasi-Periodic Solution).
...
% Moreover, CoSTAR also computed points of stability change, which are labelled in the plot as "BF". Details are saved ...
% to the field 'bifurcation' of the solution object S. In constrast to periodic solutions, however, no classification of the ...
% bifurcation points (where stability change occurs) is possible due to the lack of test functions. ...
...
solplot_options_1 = costaropts('zaxis',@(z) z(:,:,1),'space','hypertime','resolution',30,'index',[25,200]);
...
% point of the continuation above, which is the first cell of the field 's' of the solution object S.
Fc0 = S.s{1};                           % S.s is a cell array and therefore needs to be accessed accordingly
% For Hmatrix_kappa, we take the calculated Hmatrix corresponding to Fc0.
Hmatrix_kappa = S.hmatrix{1};           % S.hmatrix is a cell array and therefore needs to be accessed accordingly
%
...
options_kappa.opt_sol = costaropts('sol_type','quasiperiodic','approx_method','fourier-galerkin','cont','on','stability','on', ...
...
options_kappa.opt_approx_method = costaropts('n_FFT',2^5,'error_control','on','error_limit',[1e-4,1e-2],'ec_iter_max',10,'n_hh_max',50);
options_kappa.opt_cont = costaropts('mu_limit',mu_limit_kappa);
% We not have to set the "options.opt_stability" structure this time. We can use the default values.
%
...
%                                                                                                                                                       converges towards the bottom solution in this tutorial.
...
% which we want to approximate using the FGM. Instead of the quasi-periodic solution itself, we calculate the (image of the) torus ...
...
% which defines a curve in (n+2)-dimensional space (n depends on the dimension of the system as well as the number of harmonics). ...
% Hence, a solution Z(theta_1,theta_2) is also denoted as a "point on the curve" in the following explanations. CoSTAR will ...
% approximate discrete points on this curve by executing a path continuation. As the frequency eta is varied, the continuation ...
% parameter is mu = eta. The remaining parameters are set to:
...
% z_IV is too far away from z0 (this may happen if the numeric values of the coefficients were chosen inconveniently and/or there ...
% are not enough higher harmonics), it may happen that fsolve does not converge at all! In general, it can be quite challenging to ...
% find suitable values for the parameters of the initial value. For our Jeffcott(-Laval) Rotor example, we use:
Hmatrix = [0, 1, 0, 3, 0;       % NOTE: This Hmatrix will only lead to a good approximation if the error control is used ...
           0, 0, 1, 0, 3];      %       (we will use it).
C0 = [-0.2; -0.2; 0; 0];        % We set C0 like this because we expect a constant shift in the state variables z1 and z2 due to Fg
Cmatrix = [  0,    1.35;        % The coefficients C_3 and C_4 corresponding to Hmatrix(:,4:5) will be guessed 
           -0.35,   0;                              
           -0.95,   0;
             0,   -1.35];
Smatrix = [-0.38,   0;          % The coefficients S_3 and S_4 corresponding to Hmatrix(:,4:5) will be guessed  
             0,   -1.35;                            
             0,   -1.35;
           0.875,   0  ];
...
%
% Next, there is the "options.opt_sol" structure. There are three mandatory fields and a total of four optional-needed fields, ...
% which we all need for mixed systems exhibiting one autonomous and one non-autonomous frequency. Apart from that, there are four ...
...
%                                 fsolve to find the first approximate solution Z0(theta_1,theta2,mu0). The k-th column of 'smatrix' ...
%                                 corresponds to the (k+1)-th column of 'hmatrix' (the ordering must match the ordering in 'hmatrix'). ...
%                                 If the number of columns is less than (n_hh-1), the missing coefficients are guessed baes on an ...
%                                 exponential decay of the norm of the coefficients. We already defined Smatrix above.
%                                 -> Allowed values: [dim x n] (double) array with 2 <= n <= (n_hh-1) = size(hmatrix,2)-1
%                   - 'fc0':      Can be used INSTEAD OF 'c0', 'cmatrix' and 'smatrix', i.e. 'fc0' is not allowed when specifying ...
%                                 'c0', 'cmatrix' and 'smatrix' and vice versa. Sets the assembled Fourier series coefficient ...
...
%                                 coefficients. 'fc0' is ideal for starting from a previously calculated point on the curve.
%                                 -> Allowed values: [dim*(2*n+1) x 1] (double) array with 2 <= n <= (n_hh-1) = size(hmatrix,2)-1
% NOTE: For usage of the field 'fc0', please see section 1.4 of the "Duffing Oscillator" example above.
%                  
% So far we have defined all "options" structures which CoSTAR always needs. Since we want to calculate a quasi-periodic solution ...
% and execute a continuation, we also have to set the "options.opt_approx_method" as well as the "options.opt_cont" structures. The ...
% fields of the "options.opt_approx_method" structure depend on the solution type as well as the chosen approximation method. For ...
% quasi-periodic solutions using the FGM, there are no mandatory fields and six optional field.
options.opt_approx_method = costaropts('n_FFT',2^6,'error_control','on','error_limit',[1e-4,1e-2],'ec_iter_max',10,'n_hh_max',50,...
                                       'phase_condition','int_poincare');
% Optional fields: - 'n_FFT':          Sets the number of evaluation points along each theta-direction for the Fast Fourier ...
%                                      Transformation. For ideal performance of the FFT, only "power of two"-values should be used. ...
%                                      A high value leads to higher accuracy but also to higher numerical effort.
%                                      -> Allowed values: Power of 2
...
%                                       -> Allowed values: 'poincare', 'int_poincare'
%                                       -> Default value:  'int_poincare'
% NOTE: It is not necessary to set 'n_FFT', 'error_control', 'ec_iter_max' and 'phase_condition' since the default values are used. ...
%       However, it is demonstrated here in order to show the fields and possible values. 
%
options.opt_cont = costaropts('mu_limit',mu_limit,'step_width',0.02,'direction',-1);
% Mandatory fields: - 'mu_limit':    Sets the limits of the continuation. For this purpose, we defined the "mu_limit" variable.
%                                    -> Allowed values: [1x2] double array of increasing numerical values
...
% fields of "options.opt_init" and "options.opt_approx_method", were explained above. Concerning the rest of the "options" structures ...
% ("options.system", "options.opt_sol", "options.opt_cont" and "options.opt_stability"), only the necessary fields were defined and ...
% explained to some extend. Note that we do not have to define the options.opt_stability structure at all since we use the default values.
% If you want to have a deeper insight into the "options" structures and its fields, please use the "costarhelp" function by typing ...
% "costarhelp.options" in the command window. In order to directly open the help pages of particular "options" structures, type ...
% "costarhelp.<name_of_options_structure>", e.g. "costarhelp.opt_cont". In case of "options.opt_init", "options.opt_approx_method" ...
% and "options.opt_stability", type "costarhelp.<name_of_options_structure>('QPS','FGM')" ("QPS": Quasi-Periodic Solution).
...
% In the default plot created by CoSTAR, unstable solutions are depicted in red, while stable solutions are shown in green. ...
% Please note that all solutions are plotted in blue if the stability was not computed. ...
% Moreover, CoSTAR can compute points of stability change, which are labelled as "BF" in the plot. Details would be saved ...
% to the field 'bifurcation' of the solution object S. In this example, no change in stability was detected. Please note that ...
% no classification of the bifurcation points (where stability change occurs) is possible due to the lack of test functions. ...
% It is pointed out again that details of the computation of stability and bifurcation points are addressed in a separate tutorial ...
% and are therefore not explained here.
%
% In order to plot the hyper-time surfaces Z_i(theta_1,theta_2,mu) (torus function components), we need to call the "solplot" ...
% function. Similar to the "costar" function, solplot expects a structure that defines all required options for the plot.
solplot_options_2 = costaropts('zaxis',@(z) z(:,:,1),'space','hypertime','resolution',30,'index',[5,25]);
% Mandatory fields: - 'zaxis':       Defines what is plotted on the vertical axis. "@(z) z(:,:,1)" plots the first component (i = 1) ...
%                                    of the torus function against the hyper-time domain [0, 2*pi]^2.
%                   - 'space':       Specifies the "domain" of the plot. 'hypertime' plots the hyper-time surfaces of the solution.
% Optional fields:  - 'resolution':  Sets the number of points for the plot in each theta-direction.
...
%
% Our aim is to approximate the quasi-periodic solution of equation (8) z(tau) for epsilon in the range of [0.1, 1.25] using the FGM. ...
% Instead of the quasi-periodic solution itself, we calculate the (image of the) torus function Z = Z(theta_1,theta_2) that ...
% describes the torus on which the quasi-periodic solution evolves, using the hyper-time invariance equation:
...
% a curve in (n+3)-dimensional space (n depends on the dimension of the system as well as the number of harmonics). ...
% Hence, a solution Z(theta_1,theta_2) is also denoted as a "point on the curve" in the following explanations. CoSTAR will ...
...
%
Fcn = @(t,z,param) vdP_coupled(t,z,param);          % Fcn is a function handle containing the right-hand side (RHS) of equation (8).
...
% "time argument" being t (and not tau) is required by CoSTAR. It is equal to our dimensionless time tau.
%
% TASK: Open the function "vdP_coupled" (click the right mouse button right on it and choose 'Open "vdP_coupled"' or go into the ...
...
%
% Now we define the "param" array. It is important that this is a cell array AND the order of the parameters placed in param must ...
% correspond to the definitions in the "vdP_coupled" function! For example, alpha is defined by alpha = param{2} in the "vdP_coupled" ...
...
%
% Last but not least, we set variables which are used to complete the initial value (IV) where the nonlinear system solver ...
% fsolve starts to find the first approximate solution of equation (9) at mu0 (i.e. the first point on the curve), referred to as ...
% Z0 = Z(theta_1,theta_2,mu0). Using the FGM, the initial value is created via a multi-dimensional Fourier series:
% Z_IV (theta_1,theta_2) = sum_(k=0)^(n_hh-1) ( C_k*cos(h_1_k*theta_1+h_2_k*theta_2) + S_k*sin(h_1_k*theta_1+h_2_k*theta_2) ).
% The h_i_k (i = 1,2) are taken from Hmatrix = [h_1_0, h_1_1, ... , h_1_(n_hh-1);
%                                               h_2_0, h_2_1, ... , h_2_(n_hh-1)]
...
% be a maximum of the number of harmonics n_hh minus 1. If the number of columns is less than (n_hh-1), the remaining coefficients ...
% are guessed based on an exponential decay of the norm of the coefficients. Further details will be explained when defining the ...
% "options.opt_init" structure in section "3.2.2 The CoSTAR settings".
% It is important to note that the better z_IV matches the approximate solution z0, the faster fsolve converges towards z0. If ...
% z_IV is too far away from z0 (this may happen if the numeric values of the coefficients were chosen inconveniently and/or there ...
% are not enough higher harmonics), it may happen that fsolve does not converge at all! In general, it can be quite challenging to ...
% find suitable values for the parameters of the initial value. For our Coupled van der Pol Oscillator example, we use:
Hmatrix = [0, 1, 0, 3, 0;       % NOTE: This Hmatrix will only lead to a good approximation if the error control is used ...
           0, 0, 1, 0, 3];      %       (we will use it).
C0 = [0; 0; 0; 0];
Cmatrix = [-0.2,  0.1;          % The coefficients C_3 and C_4 corresponding to Hmatrix(:,4:5) will be guessed            
           -0.1, -1;
            2,   -0.15;
            0.2,  2.25];
Smatrix = [ 2,   -0.1;          % The coefficients S_3 and S_4 corresponding to Hmatrix(:,4:5) will be guessed
            0.2,  1.5;
            0.2, -0.15;
            0.1,  1.5];
...
% field ('non_auto_freq') is needed when the system exhibits autonomous frequency(s). Apart from that, there are four optional ...
% fields, but we use their default values and therefore do not set them.
options.opt_sol = costaropts('sol_type','quasiperiodic','approx_method','fourier-galerkin','cont','on','stability','on', ...
...
% fields of the "options.opt_approx_method" structure depend on the solution type as well as the chosen approximation method. For ...
% quasi-periodic solutions using the FGM, there are no mandatory fields and six optional field.
options.opt_approx_method = costaropts('n_FFT',2^6,'error_control','on','error_limit',[1e-3,1e-1],'ec_iter_max',10,'n_hh_max',50,...
                                       'phase_condition','int_poincare');
% Optional fields: - 'n_FFT':          Sets the number of evaluation points along each theta-direction for the Fast Fourier ...
%                                      Transformation. For ideal performance of the FFT, only "power of two"-values should be used. ...
%                                      A high value leads to higher accuracy but also to higher numerical effort.
...
%                                       -> Allowed values: 'poincare', 'int_poincare'
%                                       -> Default value:  'int_poincare'
% NOTE: It is not necessary to set 'n_FFT', 'error_control', 'error_limit', 'ec_iter_max' and 'phase_condition' since the default values ...
%       are used. However, it is demonstrated here in order to show the fields and possible values.
%
options.opt_cont = costaropts('mu_limit',mu_limit,'step_width',0.02);
% Mandatory fields: - 'mu_limit':    Sets the limits of the continuation. For this purpose, we defined the "mu_limit" variable.
%                                    -> Allowed values: [1x2] double array of increasing numerical values
% Optional fields:  - 'step_width':  Defines the (initial) step width (default: 0.1). The step width will be altered in the range ...
%                                    of step_width*[0.2, 5]. It is set here because the default step width would be too large. ...
...
% In order to plot the hyper-time surfaces Z_i(theta_1,theta_2,mu) (torus function components), we need to call the "solplot" ...
% function. Similar to the "costar" function, solplot expects a structure that defines all required options for the plot.
solplot_options_3 = costaropts('zaxis',@(z) z(:,:,1),'space','hypertime','resolution',40,'index',[5,20]);
% Mandatory fields: - 'zaxis':       Defines what is plotted on the vertical axis. "@(z) z(:,:,1)" plots the first component (i = 1) ...
%                                    of the torus function against the hyper-time domain [0, 2*pi]^2.
%                   - 'space':       Specifies the "domain" of the plot. 'hypertime' plots the hyper-time surfaces of the solution.
% Optional fields:  - 'resolution':  Sets the number of points for the plot in each theta-direction.
...
% The CoSTAR tutorial on calculating quasi-periodic solutions using the Fourier-Galerkin Method is now finished.
