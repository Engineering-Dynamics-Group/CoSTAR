...
% discretization). Hence, a solution Z(theta_1,theta_2) is also denoted as a "point on the curve" in the following explanations. ...
...
% too far away from Z0 and/or the discretization is too coarse, it may happen that fsolve does not converge at all! In general, it ...
...
%                                       calculation is not available yet when using the FDM, so we have to set it to 'off'.
...
%                                       -> Allowed values: Positive integer >= 2
%                                       -> Default value:  50
%                  - 'scheme_1':        This field refers to the discretization scheme used to approximate the derivation dZ/dtheta_1.
...
%                     but they refer to the discretization in theta_2-direction (instead of theta_1-direction).
...
%                                    is detailly explained in a separate tutorial.
...
opt_solplot = costaropts('zaxis',@(z) z(:,:,1),'space','hypertime','resolution',25,'index',[25,200]);
...
% In order to plot the hyper-time surfaces Z_i(theta_1,theta_2,mu) (torus function components), we need to call the "solplot" ...
% function. Similar to the "costar" function, solplot expects a structure that defines all required options for the plot.
opt_solplot = costaropts('zaxis',@(z) z(:,:,1),'space','hypertime','resolution',30,'index',[5,22]);
% Mandatory fields: - 'zaxis':       Defines what is plotted on the vertical axis. "@(z) z(:,:,1)" plots the first component (i = 1) ...
%                                    of the torus function against the hyper-time domain [0, 2*pi]^2.
%                   - 'space':       Specifies the "domain" of the plot. 'hypertime' plots the hyper-time surfaces of the solution.
% Optional fields:  - 'resolution':  Sets the number of points for the plot in each theta-direction.
...
% a curve in (n+3)-dimensional space (n depends on the dimension of the system as well as the finite difference discretization). ...
...
% In order to plot the hyper-time surfaces Z_i(theta_1,theta_2,mu) (torus function components), we need to call the "solplot" ...
% function. Similar to the "costar" function, solplot expects a structure that defines all required options for the plot.
opt_solplot = costaropts('zaxis',@(z) z(:,:,1),'space','hypertime','resolution',35,'index',[5,20]);
% Mandatory fields: - 'zaxis':       Defines what is plotted on the vertical axis. "@(z) z(:,:,1)" plots the first component (i = 1) ...
%                                    of the torus function against the hyper-time domain [0, 2*pi]^2.
%                   - 'space':       Specifies the "domain" of the plot. 'hypertime' plots the hyper-time surfaces of the solution.
% Optional fields:  - 'resolution':  Sets the number of points for the plot in each theta-direction.
