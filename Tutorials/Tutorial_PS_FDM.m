...
% For periodic solutions using the FDM, there are no mandatory fields and five optional field of which a maximum of three may be used.
...
%                  - 'phase_condition': Defines the phase condition (PC), which is only allowed for autonomous systems. The Poincare ...
%                                       PC is easier to compute, while the Integral PC minimises shifts in the solution profile. ...
%                                       'phase_condition' is not allowed when dealing with non-autonomous systems.
...
%       demonstrated here in order to show the fields and possible values. For usage of the field 'points', please see section 1.4 ...
%       or section 2.2 of the "van der Pol Oscillator" example below. For usage of the field 'phase_condition', please see section 2.2.
%
...
% execute a continuation, we also have to set the "options.opt_approx_method" as well as the "options.opt_cont" structures.
% The fields of the "options.opt_approx_method" structure depend on the solution type as well as the chosen approximation method. ...
% For periodic solutions using the FDM, there are no mandatory fields and four optional field of which a maximum of four may be used.
options.opt_approx_method = costaropts('n_int',100,'points',[-4,-3,-2,-1,0,1,2],'phase_condition','integral');
% Optional fields: - 'n_int':         Sets the number of intervals into which one period is divided. That means that the solution ...
%                                     z(tau,mu) is approximated by (n_int+1) points in the range of [0, T] (T: periodic time). ... 
%                                     This defines the resolution of the finite difference grid.
%                                     -> Allowed values: Positive integer >= 2
...
%                  - 'phase_condition': Defines the phase condition (PC), which is only allowed for autonomous systems. The Poincare ...
%                                       PC is easier to compute, while the Integral PC minimises shifts in the solution profile, ...
%                                       which is why the Integral PC is recommended. 'phase_condition' is not allowed when dealing ...
%                                       with non-autonomous systems.
%                                       -> Allowed values: 'poincare', 'integral'
%                                       -> Default value:  'integral'
% NOTE: It is not necessary to set the fields 'n_int' and 'phase_condition' since the default value is used. However, it is ...
%       demonstrated here in order to show the field and a possible value. For usage of the fields 'scheme' and 'approx_order', ...
%       please see the "Duffing Oscillator" example above.
%
