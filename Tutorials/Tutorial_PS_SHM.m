...
%               1.1  Introduction                   %
%
% The first example we have a look on features the Duffing equation with external forcing. The equation reads:
%                   x'' + 2*D*x' + c*x + kappa*x^3 = g*cos(eta*tau)     (1)
% where x is the deflection, D is the damping factor, c and kappa are the coefficients of the (non-)linear stiffness, g is the ...
% amplitude of the excitation, eta is the dimensionless excitation frequency and tau is the dimensionless time. Following that, ...
...
%                                             z2' = -2*D*z2 - c*z1 - kappa*z1^3 + g*cos(eta*tau) 
%
...
%
Fcn =  @(t,z,param) duffing(t,z,param); % Fcn is a function handle containing the right-hand side (RHS) of equation (2). 
...
%
% TASK: Open the function "duffing" (click the right mouse button right on it and choose 'Open "duffing"' or go into the "RHS" ...
...
% correspond to the definitions in the "duffing" function! For example, kappa is defined by kappa = param{2} in the "duffing" ...
...
% field ('auto_freq') is used instead of 'non_auto_freq' when the system is autonomous. Apart from that, there are four optional ...
...
%                                       -> Default value: 'iter'
%                   - 'log':            Defines whether a log file of the computation is returned.
%                                       -> Allowed values: 'on' or 'off'
%                                       -> Default value: 'on'
%                   - 'save':           If enabled, CoSTAR saves the DynamicalSystem object DYN and the Solution object S in a ...
%                                       mat-file after every computed solution. This can be used to save the already computed ...
%                                       data in the case that CoSTAR terminates suddenly due to an error.
%                                       -> Allowed values: 'on' or 'off'
%                                       -> Default value: 'off'
%
...
% Mandatory fields: - 'ic':  Defines the initial condition(s) (i.e. the shooting points) for the shooting method. 'ic' should be ...
%                            as close as possible to the periodic orbit.Since 'ic' consists of state space values only, ...
%                            n_shoot_ic = numel(ic)/dim must be an integer (dim: state space dimension). ...
%                               * If n_shoot_ic = 1, 'ic' is used as initial condition z(t=0) in state space. Missing initial ...
%                                 shooting points are obtained via numerical time integration.
%                               * If n_shoot_ic = n_shoot (n_shoot: see below), 'ic' is directly used as initial guess.
%                               * In all other cases (n_shoot_ic =/= n_shoot and n_shoot_ic > 1), the state space values stored in ...
%                                 'ic' are interpolated to obtain the desired shooting points.
%                            -> Allowed values: [dim*n_shoot_ic x 1] (double) array
%                            -> Default value:  (no default value)
%
...
% For periodic solutions using the SHM, there are no mandatory fields and three optional fields, from which two may be used here.
...
%                                       The Poincare PC is easier to compute, while the Integral PC minimises shifts in the solution ...
%                                       profile, which is why the Integral PC is recommended. 'phase_condition' is not allowed when ...
%                                       dealing with non-autonomous systems.
%                                       -> Allowed values: 'poincare', 'integral'
%                                       -> Default value:  'integral'
...
param_kappa = {kappa0, D, eta_kappa, g, c};
...
Fcn = @(t,z,param) vdP_auto(t,z,param);             % Fcn is a function handle containing the right-hand side (RHS) of equation (4).
...
%
% TASK: Open the function "vdP_auto" (click the right mouse button right on it and choose 'Open "vdP_auto"' or go into the ...
...
%
% Now we define the "param" array. It is important that it is a cell array AND the order of the parameters placed in param must ...
% correspond to the definitions in the "vdP_auto" function! Since epsilon is the only parameter of the RHS of (4) (apart from z), ...
...
% Next, there is the "options.opt_sol" structure. There are three mandatory fields and a total of four optional-needed fields. ...
% However, we only need three of the optional-needed fields when computing periodic solutions. The remaining optional-needed ...
% field ('non_auto_freq') is used instead of 'auto_freq' when the system is non-autonomous. Apart from that, there are four ...
...
% execute a continuation, we also have to set the "options.opt_approx_method" as well as the "options.opt_cont" structures.
% The fields of the "options.opt_approx_method" structure depend on the solution type as well as the chosen approximation method. ...
% For periodic solutions using the SHM, there are no mandatory fields and three optional fields.
options.opt_approx_method = costaropts('solver','ode45','n_shoot',2,'phase_condition','integral');
% Optional fields: - 'solver':          Sets the Matlab-proprietary numerical integration algorithm to obtain the time-dependent solution ...
%                                       z(tau,mu). Use specialised solvers for stiff ODEs, e.g. ode15s. ...
%                                       -> Allowed values: 'ode45', 'ode78', 'ode89', 'ode23', 'ode113', 'ode15s', 'ode23s', 'ode23t', 'ode23tb'
%                                       -> Default value:  'ode45'
...
%                                       -> Allowed values: 'poincare', 'integral'
%                                       -> Default value:  'integral'
% NOTE: It is not necessary to set the fields 'solver', 'n_shoot' and 'phase_condition' since the default values are used. However, ...
%       it is demonstrated here in order to show the fields and possible values.
%
